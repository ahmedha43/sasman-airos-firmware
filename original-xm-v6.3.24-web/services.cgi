...
if (IsSet($pwdogRetry)) {
	$pwdogRetry = IntVal($pwdogRetry);
}
if (IsSet($pwdogDelay)) {
	$pwdogDelay = IntVal($pwdogDelay);
}
if (IsSet($pwdogPeriod)) {
	$pwdogPeriod = IntVal($pwdogPeriod);
}
...
$unms_status = cfg_get_def($cfg, "unms.status", $unms_status);
$unms_key = cfg_get_def($cfg, "unms.uri", $unmsKey);
$unmsUrl = strtok($unms_key, "+");
if ($unmsUrl == "") {
	$unmsUrl = "-";
}
...
if ($REQUEST_METHOD=="POST" && $X_ACCESS_TOKEN == $token) {
	if ($unmsStatus != "enabled") {
		$unmsStatus = "disabled";
	}
	$unms_status = $unmsStatus;
	cfg_set($cfg, "unms.status", $unmsStatus);
...
$http_session_timeout = cfg_get_def_ne($cfg, "httpd.session.timeout", "900");
