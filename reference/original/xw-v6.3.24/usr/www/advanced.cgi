...
<?
  ...
  $autoack_disabled = 0;
  if ($polling == "enabled" && $wmode_type != 1 && ($polling_noack == 1 || $airsync == "enabled")) {
    $autoack_disabled = 1;
  }
  if (strlen($netmode)==0) {
	$netmode = $netmode_cfg;
  ...
  if (!IsSet($noise_immunity) || IntVal($noise_immunity) == 0) {
  	$noise_immunity = 28;
  }
  $noise_immunity = cfg_get_def($cfg, "radio.1.thresh62a", $noise_immunity);
  $chanbw = intval(cfg_get_def($cfg, "radio.1.chanbw", $chanbw));
  ...
  $mcast_rate = cfg_get_def($cfg, "radio.1.mcastrate", intval($mcast_rate));
