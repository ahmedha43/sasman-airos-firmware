...
$fw_downgrade_needs_current_password = 0;
...
{
	$timezone = cfg_get_def($cfg, "system.timezone", "UTC");
}
...
if ($is_fw_exist || $is_new_fw_uploaded) {
	...
	if (strlen($res) > 0) {
        ...
        if (is_downgrade_to_pre_sha512_passwd($newfwversion) && !isOldCryptedPassword($cfg)) {
            $fw_downgrade_needs_current_password = 1;
            ...
            if (strlen($CurrentPassword) != 0) {
                ...
                if ($crypted == crypt($CurrentPassword, $crypted)) {
                    $fw_downgrade_needs_current_password = 0;
					storeCurrentPassword($CurrentPassword);
                }
				# TODO: we could add error message, but currently nobody is handling it
				#else {
                    #$error_msg = dict_translate("msg_curr_passwd_wrong|Current password is wrong.");
                #}
            }
        }
        if ($fw_downgrade_needs_current_password) {
            $firmware_third_party_warning = dict_translate("msg_downgrade_need_passwd|Please enter your current password correctly to downgrade to the uploaded firmware:");
        } else {
